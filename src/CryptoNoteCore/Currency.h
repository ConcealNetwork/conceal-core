...
public:
  ...
  uint64_t maxBlockBlobSize() const { return m_maxBlockBlobSize; }
  uint64_t maxTxSize() const { return m_maxTxSize; }
  uint64_t publicAddressBase58Prefix() const { return m_publicAddressBase58Prefix; }
  uint64_t minedMoneyUnlockWindow() const { return m_minedMoneyUnlockWindow; }

  uint64_t timestampCheckWindow() const { return m_timestampCheckWindow; }
  uint64_t timestampCheckWindow(uint8_t blockMajorVersion) const {
  ...
  uint64_t timestampCheckWindow_v1() const { return m_timestampCheckWindow_v1; }
  ...
  uint64_t rewardBlocksWindow() const { return m_rewardBlocksWindow; }

  uint32_t zawyDifficultyBlockIndex() const { return m_zawyDifficultyBlockIndex; }
  uint64_t zawyDifficultyV2() const { return m_zawyDifficultyV2; }
  uint8_t zawyDifficultyBlockVersion() const { return m_zawyDifficultyBlockVersion; }

  uint64_t blockGrantedFullRewardZone() const { return m_blockGrantedFullRewardZone; }
  uint64_t minerTxBlobReservedSize() const { return m_minerTxBlobReservedSize; }

  uint64_t numberOfDecimalPlaces() const { return m_numberOfDecimalPlaces; }
  ...
  uint64_t difficultyWindow() const { return m_difficultyWindow; }
  uint64_t difficultyWindowByBlockVersion(uint8_t blockMajorVersion) const;
  uint64_t difficultyLag() const { return m_difficultyLag; }
  uint64_t difficultyCut() const { return m_difficultyCut; }
  uint64_t difficultyCutByBlockVersion(uint8_t blockMajorVersion) const;
  uint64_t difficultyBlocksCountByBlockVersion(uint8_t blockMajorVersion) const {
    if (blockMajorVersion >= BLOCK_MAJOR_VERSION_4) {
  ...
  uint64_t difficultyBlocksCount() const { return m_difficultyWindow + m_difficultyLag; }
  uint64_t difficultyBlocksCount2() const { return CryptoNote::parameters::DIFFICULTY_BLOCKS_COUNT; }
  ...
  uint64_t maxBlockSizeInitial() const { return m_maxBlockSizeInitial; }
  ...
  uint64_t lockedTxAllowedDeltaBlocks() const { return m_lockedTxAllowedDeltaBlocks; }
  ...
  uint64_t transactionMaxSize() const { return m_transactionMaxSize; }
  uint64_t fusionTxMaxSize() const { return m_fusionTxMaxSize; }
  uint64_t fusionTxMinInputCount() const { return m_fusionTxMinInputCount; }
  uint64_t fusionTxMinInOutCountRatio() const { return m_fusionTxMinInOutCountRatio; }
  ...
  bool getBlockReward(uint64_t medianSize, uint64_t currentBlockSize, uint64_t alreadyGeneratedCoins, uint64_t fee, uint32_t height,
  ...
  uint64_t maxBlockCumulativeSize(uint64_t height) const;

  bool constructMinerTx(uint32_t height, uint64_t medianSize, uint64_t alreadyGeneratedCoins, uint64_t currentBlockSize,
  ...
  bool isFusionTransaction(const Transaction& transaction, uint64_t size) const;
  bool isFusionTransaction(const std::vector<uint64_t>& inputsAmounts, const std::vector<uint64_t>& outputsAmounts, uint64_t size) const;
  ...
  uint64_t getApproximateMaximumInputCount(uint64_t transactionSize, uint64_t outputCount, uint64_t mixinCount) const;
...
private:
  ...
  uint64_t m_maxBlockBlobSize;
  uint64_t m_maxTxSize;
  uint64_t m_publicAddressBase58Prefix;
  uint64_t m_minedMoneyUnlockWindow;

  uint64_t m_timestampCheckWindow;
  uint64_t m_timestampCheckWindow_v1;
  ...
  uint64_t m_rewardBlocksWindow;

  uint32_t m_zawyDifficultyBlockIndex;
  uint64_t m_zawyDifficultyV2;
  uint8_t m_zawyDifficultyBlockVersion;

  uint64_t m_blockGrantedFullRewardZone;
  uint64_t m_minerTxBlobReservedSize;

  uint64_t m_numberOfDecimalPlaces;
  uint64_t m_coin;
  ...
  uint64_t m_difficultyWindow;
  uint64_t m_difficultyLag;
  uint64_t m_difficultyCut;
  ...
  uint64_t m_maxBlockSizeInitial;
  ...
  uint64_t m_lockedTxAllowedDeltaBlocks;
  ...
  uint64_t m_transactionMaxSize;
  uint64_t m_fusionTxMaxSize;
  uint64_t m_fusionTxMinInputCount;
  uint64_t m_fusionTxMinInOutCountRatio;
...
public:
  ...
  CurrencyBuilder& maxBlockBlobSize(uint64_t val) { m_currency.m_maxBlockBlobSize = val; return *this; }
  CurrencyBuilder& maxTxSize(uint64_t val) { m_currency.m_maxTxSize = val; return *this; }
  CurrencyBuilder& publicAddressBase58Prefix(uint64_t val) { m_currency.m_publicAddressBase58Prefix = val; return *this; }
  CurrencyBuilder& minedMoneyUnlockWindow(uint64_t val) { m_currency.m_minedMoneyUnlockWindow = val; return *this; }

  CurrencyBuilder& timestampCheckWindow(uint64_t val) { m_currency.m_timestampCheckWindow = val; return *this; }
  CurrencyBuilder& timestampCheckWindow_v1(uint64_t val) { m_currency.m_timestampCheckWindow_v1 = val; return *this; }
  ...
  CurrencyBuilder& rewardBlocksWindow(uint64_t val) { m_currency.m_rewardBlocksWindow = val; return *this; }

  CurrencyBuilder& zawyDifficultyBlockIndex(uint32_t val) { m_currency.m_zawyDifficultyBlockIndex = val; return *this; }
  CurrencyBuilder& zawyDifficultyV2(uint64_t val) { m_currency.m_zawyDifficultyV2 = val; return *this; }
  CurrencyBuilder& zawyDifficultyBlockVersion(uint8_t val) { m_currency.m_zawyDifficultyBlockVersion = val; return *this; }

  CurrencyBuilder& blockGrantedFullRewardZone(uint64_t val) { m_currency.m_blockGrantedFullRewardZone = val; return *this; }
  CurrencyBuilder& minerTxBlobReservedSize(uint64_t val) { m_currency.m_minerTxBlobReservedSize = val; return *this; }

  CurrencyBuilder& numberOfDecimalPlaces(uint64_t val);
  ...
  CurrencyBuilder& difficultyWindow(uint64_t val);
  CurrencyBuilder& difficultyLag(uint64_t val) { m_currency.m_difficultyLag = val; return *this; }
  CurrencyBuilder& difficultyCut(uint64_t val) { m_currency.m_difficultyCut = val; return *this; }
  ...
  CurrencyBuilder& maxBlockSizeInitial(uint64_t val) { m_currency.m_maxBlockSizeInitial = val; return *this; }
  ...
  CurrencyBuilder& lockedTxAllowedDeltaBlocks(uint64_t val) { m_currency.m_lockedTxAllowedDeltaBlocks = val; return *this; }
  ...
  CurrencyBuilder& transactionMaxSize(uint64_t val) { m_currency.m_transactionMaxSize = val; return *this; }
  CurrencyBuilder& fusionTxMaxSize(uint64_t val) { m_currency.m_fusionTxMaxSize = val; return *this; }
  CurrencyBuilder& fusionTxMinInputCount(uint64_t val) { m_currency.m_fusionTxMinInputCount = val; return *this; }
  CurrencyBuilder& fusionTxMinInOutCountRatio(uint64_t val) { m_currency.m_fusionTxMinInOutCountRatio = val; return *this; }
